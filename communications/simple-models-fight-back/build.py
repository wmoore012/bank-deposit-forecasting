...
f=page(5,'Ridge selected 112 banks that later had the lowest deposit growth.','', 'March → June 2024. Review capacity is hypothetical. Lowest growth can still be positive. Dots show rounded shares; each dot does not represent one bank.')
f.text(.055,.735,"Imagine you can examine 454 of 4,536 banks.\nYou want to find banks whose deposits will grow least—or shrink most—next quarter.\nRidge’s list included 112 of that quarter’s lowest-growth 454 banks; chance would find about 45.",size=15,va='top',linespacing=1.4)
...
for j,(name,h,col) in enumerate([('Random',k*k/n,G),('Ridge',int(march.loc['Ridge','Hits']),C),('Neural network',int(march.loc['MLP','Hits']),T)]):
 a=f.add_axes([.09+j*.31,.25,.24,.29]);count=round(100*h/k);a.scatter(np.tile(np.arange(10),10),np.repeat(np.arange(10)[::-1],10),c=[col]*count+['#DEDEDE']*(100-count),s=42);a.axis('off');a.set_title(name,size=18);a.text(.5,-.15,f'{h:.1f} expected' if j==0 else f'{h} found',transform=a.transAxes,ha='center',size=20,weight='bold')
f=page(6,'Deposit size alone found about 19 cases per 100 selections.','', 'Equal-quarter averages across three reused 2024 quarters; select the lowest predicted-growth 10%. Size-only Ridge was added after examining 2024. These are historical results, with no measured operational savings.')
f.text(.055,.735,"A case means a selected bank that later fell in its quarter’s lowest-growth 10%.\nUsing all five inputs raised the count to about 22 per 100 selections.\nRandom selection would find about 10. Size alone is a serious benchmark.",size=15,va='top',linespacing=1.4)
